...
def _build_tune_config(opt: str, cfg, data_yaml: Path, out_dir: Path):
    ...
    if opt == "adamw":
        ...
        search_space = {
            # ---------- LR & 스케줄 ----------
            "lr0": (2e-4, 1e-3),   # ○ 2e-4~1e-3 : 초기 LR ↓ : 적응형 scaling 덕분에 작은 값으로도 충분
            "lrf": (0.10, 0.45),   # ○ 0.10~0.45 : 최종 LR 비율 ↑ : 낮은 lr0에서 decaying 효과 확보
            "momentum": (0.85, 0.93),  # ○ 0.85~0.93 : 인공 관성 조절 핵심
            # ---------- 정규화 & 워밍업 ----------
            "weight_decay": (2e-4, 8e-4),  # ○ 2e-4~8e-4 : decoupled L2 규제 → 0.0005 주변 ±60 %
            "warmup_epochs": (0.3, 1.5),   # ○ 0.3~1.5 : 적응형 옵티마이저라 warm‑up 짧게
            "warmup_momentum": (0.80, 0.90), # ○ 0.80~0.90 : AdamW는 적응형 β₂ 덕분에 진동 완충이 강함.
            "warmup_bias_lr": (0.03, 0.20),# ○ 0.03~0.20 : bias 파라미터 variance 보정용
            # ---------- 손실 가중치 ----------
            "box": (6.0, 10.0),    # ○ 6.0~10.0 : 객체 갯수 적은 데이터에 맞춰 기본값(7.5)±30 %
            "cls": (0.40, 1.60),   # ○ 0.40~1.60 : AdamW는 grad norm이 커서 1.5 부근까지 올려도 안정.
            "dfl": (1.0, 2.4),     # ○ 1.0~2.4 : AdamW는 큰 grad도 견디므로 2.4 상한까지 탐색.
            # ---------- 색조 증강(0 고정) ----------
        ...
        search_space = {
            # ---------- LR & 스케줄 ----------
            "lr0": (3e-3, 3e-2),   # ○ 3e-3~3e-2 : 고정 step이므로 큰 LR 필요
            "lrf": (0.05, 0.20),   # ○ 0.05~0.20 : 종단 LR 비율 낮게 → 과도한 decaying 방지
            "momentum": (0.85, 0.98),  # ○ 0.85~0.98 : SGD에서는 β₁이 커져도 WD‑효과가 증가하지 않아 상대적으로 안전
            # ---------- 정규화 & 워밍업 ----------
            "weight_decay": (4e-5, 6e-4),  # ○ 4e-5~6e-4 : LR에 곱붙어 적용 → 값이 클수록 불안
            "warmup_epochs": (0.5, 2.5),   # ○ 0.5~2.5 : 큰 LR 때문에 좀 더 길게
            "warmup_momentum": (0.80, 0.95), # ○ 0.80~0.95 : 값이 낮을수록 초반 진동 억제, 너무 높으면 발산.
            "warmup_bias_lr": (0.05, 0.25), # ○ 0.05, 0.25 : 0.05↓ 수렴 느림, 0.25↑ gradient 폭주 위험.
            # ---------- 손실 가중치 ----------
            "box": (5.0, 8.0),    # ○ 5.0~8.0 : 과한 가중치 시 발산 위험 → 범위 ↓
            "cls": (0.30, 1.20),   # ○ 0.30~1.20 : 클래스 수·불균형 심할수록 ↑
            "dfl": (1.0, 2.0),      # ○ 1.0~2.0 : 박스 정밀도가 중요할수록 ↑
            # ---------- 색조 증강(0 고정) ----------
            "hsv_h": (0.00, 0.00),   # ○ 0~0.015 : ±5° Hue 변화(가짜색 최소화)
            "hsv_s": (0.00, 0.00),   # ○ 0~0.70 : 탈색~쨍한 범위
            "hsv_v": (0.00, 0.00),   # ○ 0~0.40 : 밝기 ±40 %, 실조도 한계
