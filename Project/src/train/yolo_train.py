...
def train_yolo(model: YOLO, cfg) -> YOLO:
    ...
    common_args = {
        ...
        # 증강
        "augment":          False,             # 테스트 데이터 증강 사용여부
        "auto_augment":     "auto_augment",    # 학습 데이터 증강 최적화
...
def _build_tune_config(opt: str, cfg, data_yaml: Path, out_dir: Path):
    ...
    if opt == "adamw":
        ...
        search_space = {
            # —— 학습률 & 스케줄 —— 
            "lr0":            (0.00072, 0.00085),  # ○ 2e-4~1e-3 : 초기 LR ↓ : 적응형 scaling 덕분에 작은 값으로도 충분
            "lrf":            (0.038,   0.043),    # ○ 0.10~0.45 : 최종 LR 비율 ↑ : 낮은 lr0에서 decaying 효과 확보
    
            # —— 모멘텀(beta1) —— 
            "momentum":       (0.88,   0.91),      # ○ 0.85~0.93 : 인공 관성 조절 핵심
    
            # —— 정규화 & 워밍업 —— 
            "weight_decay":   (0.00037, 0.00045),  # ○ 2e-4~8e-4 : decoupled L2 규제 → 0.0005 주변 ±60 %
            "warmup_epochs":  (0.80,   0.95),      # ○ 0.3~1.5 : 적응형 옵티마이저라 warm‑up 짧게
            "warmup_momentum":(0.86,   0.90),      # ○ 0.80~0.90 : AdamW는 적응형 β₂ 덕분에 진동 완충이 강함.
            "warmup_bias_lr": (0.09,   0.11),      # ○ 0.03~0.20 : bias 파라미터 variance 보정용
    
            # —— 손실 가중치 —— 
            "box":            (7.6,    8.0),       # ○ 6.0~10.0 : 객체 갯수 적은 데이터에 맞춰 기본값(7.5)±30 %
            "cls":            (0.50,   0.55),      # ○ 0.40~1.60 : AdamW는 grad norm이 커서 1.5 부근까지 올려도 안정.
            "dfl":            (1.20,   1.30),      # ○ 1.0~2.4 : AdamW는 큰 grad도 견디므로 2.4 상한까지 탐색.
    
            # —— 색조 증강 ——
            "hsv_h":          (0.013,  0.017),     # ○ 0~0.015 : ±5° Hue 변화(가짜색 최소화)
            "hsv_s":          (0.60,   0.65),      # ○ 0~0.70 : 탈색~쨍한 범위
            "hsv_v":          (0.38,   0.41),      # ○ 0~0.40 : 밝기 ±40 %, 실조도 한계
    
            # —— 기하 증강 —— 
            "degrees":        (0.0,    1.0),       # ○ 0~10°  : bbox 회전 오차 억제선
            "translate":      (0.095,  0.105),     # ○ 0~0.10 : 10 % 이동까지 framing 다양화
            "scale":          (0.78,   0.82),      # ○ 0.50~1.50 : 해상도·픽셀 손실 최소
            "shear":          (0.0,    0.2),       # ○ 0~2°   : 카메라 tilt 시뮬
            "perspective":    (0.0,    0.0002),    # ○ 0~0.001 : edge‑case 보강
            "flipud":         (0.0,    0.01),      # ○ 0~0.10 : 상하반전은 드뭄
            "fliplr":         (0.47,   0.50),      # ○ 0~0.50 : 좌우반전은 최대 50 %  
            # —— 혼합 증강 —— 
            "mosaic":         (0.67,   0.71),     # ○ 0~1.00 : 대표 혼합 증강
            "mixup":          (0.0,    0.01),     # ○ 0~0.30 : 30 % ↑는 bbox 혼란
            "copy_paste":     (0.0,    0.01),     # ○ 0~0.30 : 객체밀도↑ 시만
            "cutmix":         (0.0,    0.01),     # ○ 0~0.30 : label noise ↑ 주의
            # —— 채널 증강 ——
            "bgr":            (0.0,    0.01),     # ○ 0~1    : 채널스왑 on/off
        }
        ...
        search_space = {
            # ---------- LR & 스케줄 ----------
            "lr0":          (0.00750, 0.00887),  # ○ 3e-3~3e-2 : 고정 step이므로 큰 LR 필요
            "lrf":          (0.04307, 0.04518),  # ○ 0.05~0.20 : 과도한 decay 방지
    
            # ---------- 모멘텀 ----------
            "momentum":     (0.87000, 0.90267),  # ○ 0.85~0.98 : β₁이 커져도 WD 영향 적어 안전
    
            # ---------- 정규화 & 워밍업 ----------
            "weight_decay": (0.00050, 0.00053),  # ○ 4e-5~6e-4 : LR 곱붙음 → 값↑ 시 발산 위험
            "warmup_epochs":(2.735,  2.880),     # ○ 0.5~2.5   : 큰 LR일수록 예열 길게
            "warmup_momentum": (0.900, 0.940),   # ○ 0.80~0.95 : 낮게 시작해 진동 억제, 높이면 수렴 ↑/발산↑
            "warmup_bias_lr": (0.0963, 0.1130),  # ○ 0.05~0.25 : bias LR 증폭. 0.05↓ 느리고, 0.25↑ 폭주 위험
    
            # ---------- 손실 가중치 ----------
            "box":          (8.500,  8.693),     # ○ 5.0~8.0   : IoU 손실 비중
            "cls":          (0.490,  0.535),     # ○ 0.30~1.20 : 분류 손실 비중, 클래스 수·불균형↑ 시 확대
            "dfl":          (1.251,  1.322),     # ○ 1.0~2.0   : DFL 비중, 소형 객체 정밀도↑ 시 확대
    
            # ---------- 색조 증강 ----------
            "hsv_h":        (0.0145, 0.0162),    # ○ 0~0.015   : ±5° Hue 변화
            "hsv_s":        (0.7654, 0.8000),    # ○ 0~0.70    : 채도 0→70%
            "hsv_v":        (0.3784, 0.4100),    # ○ 0~0.40    : 밝기 ±40%
    
            # ---------- 기하 증강 ----------
            "degrees":      (0.0,    0.0),       # ○ 0~10°     : 회전
            "translate":    (0.090,  0.106),     # ○ 0~0.10    : 평행 이동
            "scale":        (0.750,  0.782),     # ○ 0.50~1.50 : 확대/축소
            "shear":        (0.0,    0.0),       # ○ 0~2°      : 전단
            "perspective":  (0.0,    0.0),       # ○ 0~0.001   : 원근 왜곡
            "flipud":       (0.0,    0.0),       # ○ 0~0.10    : 상하 반전 확률
            "fliplr":       (0.425,  0.425),     # ○ 0~0.50    : 좌우 반전 확률
    
            # ---------- 혼합 증강 ----------
            "mosaic":       (0.6755, 0.7300),    # ○ 0.40~0.80 : 대표 혼합 증강
            "mixup":        (0.0,    0.0),       # ○ 0.00~0.30 : 블렌딩 확률
            "copy_paste":   (0.0,    0.0),       # ○ 0.00~0.20 : 객체 복사붙여넣기 확률
            "cutmix":       (0.0,    0.0),       # ○ 0.00~0.20 : 절단·교차 확률
    
            # ---------- 채널 증강 ----------
            "bgr":          (0.0,    0.0),       # ○ 0~0.30    : 채널 스왑 확률
        }
